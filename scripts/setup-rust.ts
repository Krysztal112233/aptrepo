...
import { loadRustToolchainConfig, parseArchitecture } from "./config.ts";
import {
  capture,
  downloadVerified,
  exists,
...
import { installedRustRoot, rustDebPath } from "./toolchains.ts";
...
async function buildRustToolchain(): Promise<void> {
  const archives = await Promise.all(
    config.components.map(async (component) => {
      const filename = `${component}-${config.version}-${target.target}.tar.xz`;
  ...
  try {
    ...
    for (let index = 0; index < config.components.length; index++) {
      ...
      await run("sh", [
        join(
          extractDir,
          `${component}-${config.version}-${target.target}`,
          "install.sh",
        ),
        `--prefix=${config.installPrefix}`,
        `--destdir=${packageRoot}`,
        "--disable-ldconfig",
      ]);
    }
